...
{
    /// <summary>
    /// خوارزمية ضغط Huffman مع دعم المعالجة المتوازية
    /// </summary>
    public class HuffmanCompression : ICompressionAlgorithm
    {
        public string Name 
        { 
            get { return "Huffman Coding"; } 
        }
        ...
        public async Task<CompressionResult> CompressAsync(IEnumerable<ICompressibleItem> items, string outputPath, string password = null, IProgress<CompressionProgress> progress = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                using (var writer = new BinaryWriter(outputStream))
                {
                    ...
                    {
                        var item = itemList[i];
                        var task = HuffmanParallelProcessor.CompressItemParallelAsync(item, password, semaphore, cancellationToken);
        ...
        private async Task WriteHeaderAsync(BinaryWriter writer, List<ICompressibleItem> items, string password, CancellationToken cancellationToken)
        {
            ...
            writer.Write(Encoding.ASCII.GetBytes("CVH1")); // Compression Vault Huffman v1
